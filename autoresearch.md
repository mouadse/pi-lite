...
- Discarded prepared INSERT statement caching, explicit per-save transactions, direct vector BLOB binding, fixed cache headroom reservation, float dot accumulation, a separate slim prompt-context search path, save embedding reuse, updated_at index removal, and empty-category special casing; all regressed or were within noise versus the best kept runs.
- Kept sparse-vector caching for the deterministic hash embeddings: `total_us=18,837` (~98.9% faster than baseline). Dense vectors are still persisted for compatibility, while cached similarity scans merge non-zero entries and preserve exact cosine scores.
